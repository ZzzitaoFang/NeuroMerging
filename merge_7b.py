...
def get_merge_performance(args: argparse.Namespace, finetuned_model_names: list, merge_task_names: list,
    ...
    SAVE_MERGE_MODELS = "./merged_models/"
    ...
    if args.merge_chinese:
        save_chinese_model_path = f"{SAVE_MERGE_MODELS}/{'_'.join(merge_task_names)}/chinese_7b/{args.save_model_name}"
    if args.merge_math:
        save_math_model_path = f"{SAVE_MERGE_MODELS}/{'_'.join(merge_task_names)}/math_7b/{args.save_model_name}"
    if args.merge_code:
    ...
    save_model_paths = [save_chinese_model_path, save_math_model_path, save_code_model_path]
