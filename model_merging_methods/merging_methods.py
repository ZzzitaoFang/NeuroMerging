...
class MergingMethod:
    ...
    def neuro_merging(
        ...
        def ours_kernel(pt_w, ft_ws, key, cum_threshold=1): # K, rm, mf, lam
            if pt_w.ndim == 2:
        ...
        models_to_merge_task_vectors = [
            TaskVector(
                pretrained_model=merged_model, finetuned_model=model_to_merge, exclude_param_names_regex=exclude_param_names_regex
            ) for model_to_merge in models_to_merge
        ]

        flattened_models_to_merge_param = [
            task_vector_param_dict_to_single_vector(task_vector=task_vector) for task_vector in models_to_merge_task_vectors
        ]
        
        # Tensor, shape (num_models_to_merge, num_total_params), flattened parameters of individual models that need to be merged
        flattened_models_to_merge_param = torch.vstack(flattened_models_to_merge_param)
        
        ptm_check = {param_name: param_value for param_name, param_value in merged_model.named_parameters()}
        ft_checks = []
        with torch.no_grad():
            # Tensor, shape (num_models_to_merge, num_total_params), mask the smallest-magnitude parameter values using param_value_mask_rate
            flattened_models_to_merge_param = mask_smallest_magnitude_param_values(
            ...
            for i in range(flattened_models_to_merge_param.size(0)):
                # merged parameter dictionary
                ft_checks.append(
                    single_vector_to_task_vector_param_dict(
                        single_vector=flattened_models_to_merge_param[i], task_vector=models_to_merge_task_vectors[0]
                    )
                )
                
            del flattened_models_to_merge_param, models_to_merge_task_vectors
            gc.collect()
        ...
        return merge_dict
        #     merged_task_vector = TaskVector(task_vector_param_dict=merge_dict)
    ...
    def neuro_merging_double(
        ...
        def ours_kernel_double(pt_w, ft_ws, key, cum_threshold=1): # K, rm, mf, lam
            if pt_w.ndim == 2:
                pt_w_ = pt_w.unsqueeze(0) # (1, m, n)
            elif pt_w.ndim == 1:
                pt_w_ = pt_w.unsqueeze(0).unsqueeze(0) # (1, 1, n)
            ...
            if len(pt_w.shape) == 2:
                return proj, rep_vecs
                merged_tv = proj + rep_vecs
            else:
                return proj.squeeze(0), rep_vecs.squeeze(0)
                merged_tv = proj.squeeze(0) + rep_vecs.squeeze(0)
        ...
        with torch.no_grad():
            ...
            for key, ptm_value in tqdm_ptm_check:
                ...
                if this_operator == "neuronal_behavior":
                    adj_weight1, adj_weight2 = ours_kernel_double(
                        ptm_value, 
                        ft_values,
                        key, 
                    )
                    ...
                    adj_weight1 = adj_weight2 = adj_weight / 2
                else:
                    raise ValueError(f"Operator {this_operator} is not valid.")

                merge_dict1[key] = adj_weight1
                merge_dict2[key] = adj_weight2
        
        return merge_dict1, merge_dict2
    ...
    def merging_models(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list,
        ...
        if self.merging_method_name == "average_merging":
            ...
            merged_params = self.ties_merging(merged_model=merged_model, models_to_merge=models_to_merge,
                                              ...
                                              param_value_mask_rate=param_value_mask_rate,
                                              scaling_coefficient=scaling_coefficient)
        elif self.merging_method_name == "neuro_merging":
            # ##### Lite Mode #####
            # merged_params = self.neuro_merging(
            ...
            ##### Full Mode #####
            merge_dict1, merge_dict2 = self.neuro_merging_double(
                merged_model=merged_model, 
                models_to_merge=models_to_merge,
                exclude_param_names_regex=exclude_param_names_regex,
                param_value_mask_rate=param_value_mask_rate,
            ...
            if mask_apply_method == "average_merging":
                ...
                merged_params = self.ties_merging(merged_model=merged_model, models_to_merge=new_models_to_merge,
                                                  ...
                                                  param_value_mask_rate=param_value_mask_rate,
                                                  scaling_coefficient=scaling_coefficient)
            elif mask_apply_method == "neuro_merging":
                # ##### Lite Mode #####
                # merged_params = self.neuro_merging(
                # merged_model=merged_model, 
                # models_to_merge=models_to_merge,
    ...
    def get_merged_model(self, merged_model: nn.Module, models_to_merge: list, exclude_param_names_regex: list,
        ...
        # merged_params = self.merging_models(merged_model=merged_model, 
        #                                     models_to_merge=models_to_merge,
        #                                     exclude_param_names_regex=exclude_param_names_regex, 
        #                                     trainers=trainers,
        #                                     nums_fisher_examples=nums_fisher_examples,
        ...
        ##### Full Mode #####
        merge_dict1, merge_dict2 = self.merging_models(merged_model=merged_model, 
                                            models_to_merge=models_to_merge,
                                            exclude_param_names_regex=exclude_param_names_regex, 
                                            trainers=trainers,
                                            nums_fisher_examples=nums_fisher_examples,
        ...
        self.copy_params_to_model(params=merge_dict1, model=merged_model)
        self.copy_params_to_model(params=merge_dict2, model=merged_model2)
        
        del merge_dict1, merge_dict2
        
        return merged_model, merged_model2