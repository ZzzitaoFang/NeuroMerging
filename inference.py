...
def recover_from_pretrained_model(
    topk, 
    ...
    finetuned_model = AutoModelForCausalLM.from_pretrained(
        pretrained_model_name_or_path=os.path.join(FT_MODEL_PATH, topk, "math_7b"), device_map="cpu"
    )
    ...
    finetuned_tokenizer = AutoTokenizer.from_pretrained(
        pretrained_model_name_or_path=os.path.join(FT_MODEL_PATH, topk, "math_7b"),
    )
    ...
    with torch.no_grad():
        for param_name in finetuned_param_dict.keys():
            pretrained_param_dict[param_name] = finetuned_param_dict[param_name] * lam + pretrained_param_dict[param_name]
    ...
    finetuned_model.save_pretrained(save_directory=tmp_model_save_path)
    finetuned_tokenizer.save_pretrained(save_directory=tmp_model_save_path)
...
def test_CMMLU(tmp_model_save_path, args, logger: logging.Logger):
    tokenizer = LlamaTokenizer.from_pretrained("/path/to/chinese_7b") # Specific for Chinese_llama_alpaca
...
if __name__ == "__main__":
    ...
    for lam in lambdas:
        ...
        with open(output_file, "a+") as f:
            f.write(f"{args.topk}_{lam}:{acc_cmmlu},{acc_gsm8k},{acc_human}" + "\n")
